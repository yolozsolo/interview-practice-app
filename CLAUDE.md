...
- `docs/AI.md` stores compact project context and should be kept current.
- `.claude/commands/interview-challenge.md` defines the workflow for creating and grading practice tasks.
- `.claude/commands/references/` contains supporting grading material.
...
For generated challenge files, follow the section markers described in `.claude/commands/interview-challenge.md` and keep the solution out of starter files.
...
Use concise imperative commit messages such as `Add delta merge challenge` or `Update grading rubric`. Pull requests should include the purpose, files changed, validation commands run, and any known limitations. For challenge changes, mention the target topic, difficulty, and expected runtime.
...
This project defines the following slash commands in `.claude/commands/`:

- `/save-progress` — end-of-session commit and push with progress notes
- `/interview-challenge` — generate or grade a single-file 60-minute interview challenge
